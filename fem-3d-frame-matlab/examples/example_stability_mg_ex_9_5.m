...
sections.A  = [2.5*1e4*1e-6; 1.76*1e4*1e-6]   ;        % Plocha [m²]
sections.Iy = [6.36*1e8*1e-12; 8.61*1e8*1e-12 ];         % Moment setrvačnosti k ose y [m⁴]
sections.Iz = sections.Iy  ;      % Moment setrvačnosti k ose z [m⁴]
sections.Ix = sections.Iy + sections.Iz;         % Polární moment (= Iy + Iz) [m⁴]
sections.E  = [200e9;200e9];      % [Pa]
sections.v  = [0.3;0.3];
...
kinematic.x.nodes  = [1];
kinematic.y.nodes  = [1; 3; 5];
kinematic.z.nodes  = [1; 3; 5];
kinematic.rx.nodes = [1; 3; 5];
kinematic.ry.nodes = [];
kinematic.rz.nodes = [1; 3; 5];

%% PRUTY
beams.nodesHead = [1; 2; 2; 4];
beams.nodesEnd  = [2; 3; 4; 5];
beams.sections  = [1; 2; 1; 2];
...
loads.x.nodes  = [2;4];   loads.x.value  = [-alpha*F_ref;-alpha*F_ref];
...
fprintf('\n');
fprintf('=== McGuire: Matrix structural analysis - Example 9.5 ===\n\n');
fprintf('  Reference critical load   : F_ref = 6630.00 kN\n');
fprintf('\n');
