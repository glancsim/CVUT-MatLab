...
Fmin = -diag(min(kron( ones(nr,1) .* (elements.E*pi^2*((lengths.*Lcrit).^-2)) , crossSectionsSet.I), kron(maximumStress,crossSectionsSet.A)));
...
lb(nr*na+nd+1:nr*na*2+nd) = -min(kron( ones(nr,1) .* (elements.E*pi^2*((lengths.*Lcrit).^-2)) , crossSectionsSet.I), kron(maximumStress,crossSectionsSet.A));
%% 
% *Upper bound*
...
vectorX = (value(x)).* repmat(1:na, 1, ng)';
sections = vectorX(abs(value(x)-1)<0.01) ;   
sections([4, 8, 12, 16]) = [];
sections
...
set(p, {'LineWidth'}, num2cell(5*value(crossSections(nonzeroAreas))/max(value(crossSections))));
