...
**Dva paralelní moduly:**

| Složka | Účel |
|--------|------|
| `Diplomka/` | Původní kód (diplomová práce). Testy 1–12 v `stability-tests/`. Funkce v `Resources/`. |
| `fem-stability-matlab/` | Refaktorovaná verze jako samostatná knihovna. `src/` = funkce, `tests/` = testy, `examples/` = ukázky. |

Funkce jsou v obou místech víceméně identické — změny se typicky dělají na obou místech naráz.
